...
from src.extract_fuctions.requests_fuctions import create_session
    ...
    for attempt in range(max_attempts):
        ...
        while page_num <= max_pages:
            try:
                ...
                elif (page_num - 1) % max_pages_per_session == 0:
                    logging.info(
                        Fore.YELLOW
                        + f"Reabrindo uma nova sessão após {max_pages_per_session} requisições..."
                        + Style.RESET_ALL
                    )
                    session.close()
                    break
